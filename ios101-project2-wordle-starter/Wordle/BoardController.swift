...
class BoardController: NSObject,
  ...
  private func applyNumLettersSettings(with settings: [String: Any]) {
    ...
    if let numLetters = settings[kNumLettersKey] as? Int{
        self.numItemsPerRow = numLetters // unwrap successfully
    } else{
      print("numLetters is not Int") 
    }
    // END YOUR CODE HERE
  }
  ...
  private func applyNumGuessesSettings(with settings: [String: Any]) {
    if let numGuesses = settings[kNumGuessesKey] as? Int {
        ...
        print("Failed to get number of guesses from settings")
    }
  }
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    ...
    if let rawTheme = settings[kWordThemeKey] as? String {
        if let theme = WordTheme(rawValue: rawTheme) {
            self.goalWord = WordGenerator.generateGoalWord(with: theme) // unwrap successfully
        }
    } else {
        print("Failed to get theme from settings") //unwrap failed
    }
    // END YOUR CODE HERE
  }
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
    if let alien = settings[kIsAlienWordleKey] as? Bool{
        self.isAlienWordle = alien
    } else{
        print("Failed to get isAlienWordle from settings")
    }
    // END YOUR CODE HERE
  }
}

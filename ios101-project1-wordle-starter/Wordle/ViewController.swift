...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    keyboardController.didSelectString = { [weak self] word in // self-> means ViewController obj
        if word == DELETE_KEY {
            self?.boardController.deleteLastCharacter() // if there's self, exec deleteLastCharacter func, if not->ignore(will not crash the code)
        } else {
            self?.boardController.enter(word) // if there's self, exec enter function, if not->ignore
        }
    }
    // END YOUR CODE HERE
  }

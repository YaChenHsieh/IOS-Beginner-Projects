...
class BoardController: NSObject,
  ...
  var numGuesses = 0 // how many letters have been entered.
  var currRow: Int {
  ...
  func enter(_ string: String) {
    ...
    UIView.animate(withDuration: 0.1,
      ...
      // START YOUR CODE HERE
        cell.transform = cell.transform.scaledBy(x: 1.05,
                                y: 1.05) // animated the cell by expand 1.05 size ->  The animation animates between the original transform and this new scaled transform
      // END YOUR CODE HERE
...
//    guard numGuesses > 0 && (numGuesses % numItemsPerRow != 0 || numGuesses == currRow * numItemsPerRow) else { return }
    guard numGuesses > 0 && numGuesses % numItemsPerRow != 0 else { return } // remainder -> position of the current guess within its row. if numGuesses % numItemsPerRow = 0 -> When this equals 0, it means we're at the start of a row
    // else { return } -> If deleting is not allowed, return immediately exits the function

...
class ForecastViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    let taipei = Location(name: "Taipei", latitude: 25.105497, longitude: 121.597366)
    locations = [sanJose, manila, italy, taipei]
  ...
  private func changeLocation(withLocationIndex locationIndex: Int) {
    guard locationIndex < locations.count else { return } // make sure the index do not out of bound
    let location = locations[locationIndex] // get the index location
    locationLabel.text = location.name // update the location name on screen
